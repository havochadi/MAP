...
Expected: `PASS: a coach session resolves to the correct CurrentCoach shape ...`
...
async function main() {
  ...
  const { error: assignError } = await farhan
    .from("ClassAssignment")
    .insert({ id: crypto.randomUUID(), coachId: fnData.coachId, classId: (await prisma.class.findFirstOrThrow()).id });
  if (!assignError) {
  ...
  await prisma.$disconnect();
  console.log("PASS: Coaches module queries/writes behave correctly under RLS, including the admin-create-coach Edge Function call");
}
...
async function main() {
  ...
  await prisma.$disconnect();
  console.log("PASS: Students module scopes profile/list visibility correctly under RLS; admin-only writes are enforced");
}
